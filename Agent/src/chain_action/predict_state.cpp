...
/*	const PlayerPtrCont::const_iterator o_end = rhs.ourPlayers().end();
	for ( PlayerPtrCont::const_iterator o = rhs.ourPlayers().begin();
			o != o_end;
			++o ){
		if((*o)->unum()==ball_holder_unum)continue;
	    M_our_players[ (*o)->unum() - 1 ]
	        = PredictPlayerObject::Ptr( new PredictPlayerObject( *(M_our_players[ (*o)->unum() - 1 ] ),
	                                                             Strategy::i().getPosition((*o)->unum()) ) );
	}*/
    updateLines();
}
