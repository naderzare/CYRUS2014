...
static const double GOALIE_PASS_EVAL_THRESHOLD = 30;
static const double BACK_PASS_EVAL_THRESHOLD = 25;
static const double PASS_EVAL_THRESHOLD = 18.5;
static const double CHANCE_PASS_EVAL_THRESHOLD = 18.5;
static const double OPP_PENALTY_AREA_PASS_THRESHOLD =20;//nader
...
static const double NEAR_PASS_DIST_THR = 3.0;
...
static const double OPPONENT_DIST_THR2 = std::pow( 4.0, 2 );
                               ...
                               const double & first_ball_speed) const
{
    ...
    if ( to.pos().x >= state.offsideLineX() + 1 )//0->1 nader (momken hast ayande dar ofside nabashe)
    {
#ifdef DEBUG_PRINT
        dlog.addText( Logger::PASS,
    ...
    const double OVER_TEAMMATE_IGNORE_DISTANCE2
        = std::pow( pass_dist + ( receive_point.x >= +25.0 ? 0.0 : 2.0 ), 2 );
          ...
          ++opp )
    {
        ...
        double opponent_dist_thr2=OPPONENT_DIST_THR2;
        if ( state.ball().pos().x > 25 || receive_point.x > 25 )
        	opponent_dist_thr2=std::pow( 3.5, 2 );
        ...
        if ( opp_move_dist < receiver_move_dist * 0.75 )
        {
            return false;
        }
    ...
    if ( state.ball().pos().x > 25 || receive_point.x > 25 )
    {
    	eval_threshold = OPP_PENALTY_AREA_PASS_THRESHOLD;
    }
    if ( from.goalie() )
    {
