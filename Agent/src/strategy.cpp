...
const std::string Strategy::BEFORE_KICK_OFF_CONF2 = "before-kick-off2.conf";
const std::string Strategy::NORMAL_FORMATION_CONF2 = "normal-formation2.conf";
const std::string Strategy::DEFENSE_FORMATION_CONF2 = "defense-formation2.conf";
const std::string Strategy::OFFENSE_FORMATION_CONF2 = "offense-formation2.conf";
const std::string Strategy::GOAL_KICK_OPP_FORMATION_CONF2 = "goal-kick-opp2.conf";
const std::string Strategy::GOAL_KICK_OUR_FORMATION_CONF2 = "goal-kick-our2.conf";
const std::string Strategy::GOALIE_CATCH_OPP_FORMATION_CONF2 = "goalie-catch-opp2.conf";
const std::string Strategy::GOALIE_CATCH_OUR_FORMATION_CONF2 = "goalie-catch-our2.conf";
const std::string Strategy::KICKIN_OUR_FORMATION_CONF2 = "kickin-our-formation2.conf";
const std::string Strategy::SETPLAY_OPP_FORMATION_CONF2 = "setplay-opp-formation2.conf";
const std::string Strategy::SETPLAY_OUR_FORMATION_CONF2 = "setplay-our-formation2.conf";
const std::string Strategy::INDIRECT_FREEKICK_OPP_FORMATION_CONF2 = "indirect-freekick-opp-formation2.conf";
const std::string Strategy::INDIRECT_FREEKICK_OUR_FORMATION_CONF2 = "indirect-freekick-our-formation2.conf";
...
Strategy::read( const std::string & formation_dir )
{
	...
	// before kick off
	M_before_kick_off_formation2 = readFormation( configpath + BEFORE_KICK_OFF_CONF2 );
	if ( ! M_before_kick_off_formation2 )
	{
		std::cerr << "Failed to read before_kick_off formation2" << std::endl;
		return false;
	}

	///////////////////////////////////////////////////////////
	M_normal_formation2 = readFormation( configpath + NORMAL_FORMATION_CONF2 );
	if ( ! M_normal_formation2 )
	{
		std::cerr << "Failed to read normal formation2" << std::endl;
		return false;
	}

	M_defense_formation2 = readFormation( configpath + DEFENSE_FORMATION_CONF2 );
	if ( ! M_defense_formation2 )
	{
		std::cerr << "Failed to read defense formation2" << std::endl;
		return false;
	}

	M_offense_formation2 = readFormation( configpath + OFFENSE_FORMATION_CONF2 );
	if ( ! M_offense_formation2 )
	{
		std::cerr << "Failed to read offense formation2" << std::endl;
		return false;
	}

	M_goal_kick_opp_formation2 = readFormation( configpath + GOAL_KICK_OPP_FORMATION_CONF2 );
	if ( ! M_goal_kick_opp_formation2 )
	{
		return false;
	}

	M_goal_kick_our_formation2 = readFormation( configpath + GOAL_KICK_OUR_FORMATION_CONF2 );
	if ( ! M_goal_kick_our_formation2 )
	{
		return false;
	}

	M_goalie_catch_opp_formation2 = readFormation( configpath + GOALIE_CATCH_OPP_FORMATION_CONF2 );
	if ( ! M_goalie_catch_opp_formation2 )
	{
		return false;
	}

	M_goalie_catch_our_formation2 = readFormation( configpath + GOALIE_CATCH_OUR_FORMATION_CONF2 );
	if ( ! M_goalie_catch_our_formation2 )
	{
		return false;
	}

	M_kickin_our_formation2 = readFormation( configpath + KICKIN_OUR_FORMATION_CONF2 );
	if ( ! M_kickin_our_formation2 )
	{
		std::cerr << "Failed to read kickin our formation2" << std::endl;
		return false;
	}

	M_setplay_opp_formation2 = readFormation( configpath + SETPLAY_OPP_FORMATION_CONF2 );
	if ( ! M_setplay_opp_formation2 )
	{
		std::cerr << "Failed to read setplay opp formation2" << std::endl;
		return false;
	}

	M_setplay_our_formation2 = readFormation( configpath + SETPLAY_OUR_FORMATION_CONF2 );
	if ( ! M_setplay_our_formation2 )
	{
		std::cerr << "Failed to read setplay our formation2" << std::endl;
		return false;
	}

	M_indirect_freekick_opp_formation2 = readFormation( configpath + INDIRECT_FREEKICK_OPP_FORMATION_CONF2 );
	if ( ! M_indirect_freekick_opp_formation2 )
	{
		std::cerr << "Failed to read indirect freekick opp formation2" << std::endl;
		return false;
	}

	M_indirect_freekick_our_formation2 = readFormation( configpath + INDIRECT_FREEKICK_OUR_FORMATION_CONF2 );
	if ( ! M_indirect_freekick_our_formation2 )
	{
		std::cerr << "Failed to read indirect freekick our formation2" << std::endl;
		return false;
	}
...
Strategy::updateSituation( const WorldModel & wm )
{
	...
	if ( opp_min <= our_min - 2
			|| ( opp_min <= our_min && wm.ball().pos().x < 10 ))
	{
		dlog.addText( Logger::TEAM,
...
Strategy::getFormation( const WorldModel & wm ) const
{
	...
	if((((wm.opponentTeamName()=="HELIOS2014")&&(cyrus().amIwin(wm,0)||cyrus().amIlost(wm,2)))||
			((wm.opponentTeamName()=="Gliders2014"||wm.opponentTeamName()=="Oxsy"||wm.opponentTeamName()=="YuShan2014")&&cyrus().amIwin(wm,1)))&&true){
		...
		if ( wm.gameMode().type() == GameMode::PlayOn )
		{
			switch ( M_current_situation ) {
				case Defense_Situation:
					return M_defense_formation2;
				case Offense_Situation:
					return M_offense_formation2;
				default:
					break;
			}
			return M_offense_formation2;
				...
				|| wm.gameMode().type() == GameMode::CornerKick_ )
		{
			if ( wm.ourSide() == wm.gameMode().side() )
			{
				...
				return M_kickin_our_formation2;
			}
			else
			{
				return M_setplay_opp_formation2;
			}
		}
		...
		{
			return M_indirect_freekick_our_formation2;
		}
		...
		if ( ( wm.gameMode().type() == GameMode::BackPass_
				...
				|| ( wm.gameMode().type() == GameMode::IndFreeKick_
						&& wm.gameMode().side() == wm.theirSide() ) )
		{
			return M_indirect_freekick_opp_formation2;
		}
				...
				|| wm.gameMode().type() == GameMode::FoulPush_ )
		{
			...
			else
			{
				...
				{
					return M_setplay_our_formation2;
				}
			}
		}
		...
		if ( wm.gameMode().type() == GameMode::GoalKick_ )
		{
			if ( wm.gameMode().side() == wm.ourSide() )
			{
				return M_goal_kick_our_formation2;
			}
			else
			{
				return M_goal_kick_opp_formation2;
			}
		}
		...
		if ( wm.gameMode().type() == GameMode::GoalieCatch_ )
		{
			if ( wm.gameMode().side() == wm.ourSide() )
			{
				return M_goalie_catch_our_formation2;
			}
			else
			{
				return M_goalie_catch_opp_formation2;
			}
		}
		...
		if ( wm.gameMode().type() == GameMode::BeforeKickOff
				|| wm.gameMode().type() == GameMode::AfterGoal_ )
		{
			return M_before_kick_off_formation2;
		}
...
Strategy::get_normal_dash_power( const WorldModel & wm )
{
	...
	else if ( wm.self().stamina() < ServerParam::i().staminaMax() * 0.35 )
	{
		s_recover_mode = true;
	}
	else if ( wm.self().stamina() > ServerParam::i().staminaMax() * 0.55 )
	{
		s_recover_mode = false;
	}
