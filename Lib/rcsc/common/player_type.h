...
private:
    ...
    void updateCyrusTable() ;

public:
     double distanceTable[19][12][50] ;//angleDif from body// first speed*10-->0 ta 11 // cycle
     int playerTypeForCyrus =-1;
      void settype(int i){
    	 this->playerTypeForCyrus = i;
     }
     const int gettype() const{
    	 return playerTypeForCyrus;
     }

    int nn(const WorldModel & wm)const;

    int nnn(const AbstractPlayerObject * p);
    int nnnn(bool x);
    int cyruscycle2target(	const WorldModel & wm,
    ...
    double getbonus(const AbstractPlayerObject * p,
    									Vector2D target,bool havback,
    									int kick_count)const;
    double getpenalty(const AbstractPlayerObject * p,
    									Vector2D target,bool havback,
    									int kick_count)const;

    double estimate_virtual_dash_distance( const rcsc::AbstractPlayerObject * player,double magic ,bool teammate=false )const;
    int cyruscycleturnturndash(	const AbstractPlayerObject * p,
    									Vector2D target,
    									double distThr,
    									double anglethr,
    									bool istm,
    ...
    int cyruscycleturnorivdash(	const AbstractPlayerObject * p,
    									Vector2D target,
    									double distThr,
    									double anglethr,
    									bool istm,
    ...
    int cycleTurn(	const AbstractPlayerObject * p,
    							Vector2D target,
    							double angleThr,
    							Vector2D * newSpeed,
    							Vector2D * newPos,
