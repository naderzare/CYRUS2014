...
Bhv_ChainAction::execute( PlayerAgent * agent )
{
	...
	if ( wm.existKickableTeammate()
			&& wm.teammatesFromBall().front()->unum() > wm.self().unum() )
		return true;
	...
	case CooperativeAction::Dribble:
	{
		...
		/*if (wm.opponents().size()>0)
			if ( wm.opponentsFromBall().front()->distFromBall() < 2.5
					&& first_action.dashCount() < 3 )
				if ( Body_Dribble2008( first_action.targetPoint(),0.1,ServerParam::i().maxDashPower(),first_action.dashCount(),true).execute(agent) ){
					return true;
				}*/
	...
	case CooperativeAction::Hold:
	{
		...
		if ( wm.ball().pos().x < -SP.pitchHalfLength() + 12.0 //8 nader
		...
		agent->addSayMessage( new DribbleMessage(wm.ball().pos() , 1));
		agent->setNeckAction( new Neck_ScanField() );
		return true;
		break;
	}
