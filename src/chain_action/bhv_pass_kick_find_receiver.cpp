...
/*        Vector2D nextBall = Vector2D::polar2vector(pass.firstBallSpeed(),(pass.targetPoint()-wm.ball().pos()).th());
        Vector2D velBall = Vector2D::polar2vector(pass.firstBallSpeed(),(pass.targetPoint()-wm.ball().pos()).th());
        if ( pass.targetPoint().dist(wm.ball().pos())>9 )agent->addSayMessage( new BallMessage(nextBall,velBall*0.96));
        else */agent->addSayMessage( new PassMessage( receiver_unum,
