...
Bhv_GoalieBasicMove::execute( PlayerAgent * agent )
{
    ...
    {
        // execute emergency action                  /// bere to pash age tak be tak mishe
        return true;
    }
    //----------------------------------------------------------
...
Bhv_GoalieBasicMove::doOffensivePositioning( PlayerAgent * agent )
{
...

  {
   ...
   const rcsc::Vector2D goal_left_post( ( goal_center.x ),
                                        ( goal_half_width ) );
   const rcsc::Vector2D goal_right_post( (goal_center.x ),
                                         (-goal_half_width ) );
   ...
   bool is_shoot_ball = ( ( (goal_left_post - wm.ball().pos() ).th()
                           ...
                           && ( ( goal_right_post - wm.ball().pos() ).th()
                                - wm.ball().vel().th() ).degree() > 0 );
   double self_reach = wm.interceptTable()->selfReachCycle();

   if ( is_shoot_ball
         && wm.ball().inertiaPoint( self_reach ).x
            < wm.self().pos().x - 2 )
    {
       Body_GoToPoint( Vector2D(-52.5 , wm.ball().inertiaPoint( self_reach ).y),
                                              0.8, ServerParam::i().maxPower() ).execute( agent );
       agent->setNeckAction( new Neck_TurnToBallOrScan() );
       return true;
    }
   if ( ( wm.ball().pos()+wm.ball().vel() ).x <=  PredictState(wm).ourDefenseLineX() - 8  )
    {
       Body_GoToPoint( Vector2D(-52.5 , getTargetPoint( agent ).y ),
                                              0.8, ServerParam::i().maxPower() ).execute( agent );
       agent->setNeckAction( new Neck_TurnToBallOrScan() );
       return true;
...
Bhv_GoalieBasicMove::doOffensiveIntercept( PlayerAgent * agent )
{
 ...
 if (  ball.x <=  wm.ourDefenseLineX() - 4  &&  wm.existKickableOpponent() 
    && myCycles < tmmCycles - 4  && wm.gameMode().type() == GameMode::PlayOn
 ...
 if (  ball.x <=  wm.ourDefenseLineX() - 10  &&  wm.existKickableOpponent()
    && myCycles < tmmCycles - 4  && wm.gameMode().type() == GameMode::PlayOn
    && !wm.existKickableTeammate() )
  {
      Body_Intercept( false ).execute( agent );
  ...
  if (  ball.x <=  wm.ourDefenseLineX()  &&  wm.existKickableOpponent()
    && myCycles < tmmCycles   && wm.gameMode().type() == GameMode::PlayOn
    && !wm.existKickableTeammate() 
    && myCycles <= 3
    && wm.self().pos().x > -40 )
  {
                                       ...
                                       Vector2D &move_point )
{
  ...
  if ( !wm.existKickableTeammate () && ball.x < 0 &&  wm.ourDefenseLineX() < 0 &&
       std::fabs( wm.ourDefenseLineX() - ball.x ) < 7 )
  {
    return;
  }
  if( ball.x > -22.0 )
  {
     if( ball.x > 25.0 )
       ...
       move_point = Vector2D( -24.0,0.0 );
     else if( ball.x > -5.0 )
       move_point = Vector2D( -20.0,0.0 );
     else if( ball.x > -15.0 )
       move_point = Vector2D( -30.0,0.0 );
     else if( ball.x > -22.0 )
       move_point = Vector2D( -38.0,0.0 );


     if( ball.absY() > 11.0 )
       move_point.y = sign(ball.y) * 7.0;
     if( ball.absY() > 18.0 )
       move_point.y = sign(ball.y) * 9.5;
     if( ball.absY() > 23.0 )
       move_point.y = sign(ball.y) * 11.0;

     if( ball.x < -15.0 )
     {
        if( move_point.y > 7.5 )  move_point.y = 7.5;
        if( move_point.y < -7.5 ) move_point.y = -7.5;
     }

     if( wm.ourDefenseLineX() - 15.0 < move_point.x )
       move_point.x = wm.ourDefenseLineX() - 15.0;
...
//---------------------------------------------------------//
    ...
    if( wm.teammatesFromSelf().front()->pos().x < -48.0 )
      move_y *= 1.4;

    if( move_y < -6.0 ) move_y = -6.0;
    if( move_y >  6.0 ) move_y =  6.0;
...
Bhv_GoalieBasicMove::doPrepareDeepCross( PlayerAgent * agent,
                                         Vector2D & move_point )
{
    if ( move_point.absY() < ServerParam::i().goalHalfWidth() - 2 ) //0.8
    {
    ...
    if ( ball.absY() < ServerParam::i().goalHalfWidth()*2 )
     {
      ...
      if( move_point.x < -51.2 )
        move_point.x = -51.2;
    
      if( move_point.x > -49.7 )
        move_point.x = -49.7;
     }
    else
     {
       move_point = Vector2D ( - ServerParam::i().pitchHalfLength() , ServerParam::i().goalHalfWidth() );
       if ( ball.y < 0 ) move_point.y *= -1;

       double maxDist = wm.self().playerTypePtr()->playerSpeedMax() +  agent->world().self().playerTypePtr()->reliableCatchableDist() - 0.05;
       int n = 1;//wm.ball().CycleToReachDistance ( ball.absY() - move_point.absY() ) - 2;
       //if ( n > 3 ) n=3;
       while ( wm.ball().inertiaTravel(n).absY() > move_point.absY() )
       {
          n++;
       }
       if ( n > 3 ) n=3;
       ...
       move_point = move_point + Vector2D::polar2vector( maxDist*n ,( ( rcsc::Vector2D ( ball.x , (ball.y+move_point.y)/2 ) - move_point).dir().degree() ) );
       if ( move_point.x >= -48 )
         move_point.x = -49 ;
