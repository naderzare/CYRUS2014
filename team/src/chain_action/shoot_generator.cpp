                             ...
                             const Vector2D & target_point )
{
    ...
    while ( ! over_max )
    {
        ...
        const AbstractPlayerObject * absself=&(wm.self());
		...
		if ( wm.interceptTable()->opponentReachCycle() < 3
				&& kick_count > 1 ) break;
		if ( wm.interceptTable()->opponentReachCycle() < 4
				&& kick_count > 2 ) break;
                             ...
                             const double & ball_move_dist )
{
          ...
          ++o )
    {
        ...
        if ( (*o)->goalie() )
        {
            if ( maybeGoalieCatch( *o, course,wm ) )
            {
#ifdef DEBUG_PRINT
                dlog.addText( Logger::SHOOT,
        ...
        if ( opponentCanReach( *o, course,wm ) )
        {
#ifdef DEBUG_PRINT
                dlog.addText( Logger::SHOOT,
...
//        if ( cycle > 1 )
        ...
        int l_dash = ptype->cyclesToReachDistance( dash_dist );
        int c_dash = cyrus().cycleToReachDist(wm,goalie->unum(),false,dash_dist,false);

		bool not_start_from0 = false;
		if ( n_turn == 0 &&
				( goalie->posCount() <=1 ||
						( ( ( ( ball_pos - goalie->pos() ).th() - ( goalie->vel().th() ) ).abs() < 15 ) ) ) ){
			if ( goalie->vel().r() > 0.1 ){
				not_start_from0 = true;
			}
		}

        int n_dash =  (not_start_from0 && c_dash!=1000) ? c_dash : l_dash;
        ...
        int bonthr = 5;
        if(wm.interceptTable()->opponentReachCycle()<3)bonthr=2;
        int bonus_step = ( in_penalty_area
                           ? bound( 0, goalie->posCount(), bonthr )
...
// #endif
    ...
    for ( int cycle = min_cycle; cycle < max_cycle; ++cycle )
    {
        ...
        if ( cycle > 1 )
        {
            dash_dist -= control_area*0.99;
        }


        int l_dash = ptype->cyclesToReachDistance( dash_dist );
        int c_dash = cyrus().cycleToReachDist(wm,opponent->unum(),false,dash_dist,false);
		...
		if ( n_turn == 0 &&
				( opponent->posCount() <=1 ||
						( ( ( ( ball_pos - opponent->pos() ).th() - ( opponent->vel().th() ) ).abs() < 15 ) ) ) ){
			if ( opponent->vel().r() > 0.1 ){
				not_start_from0 = true;
			}
		}
